...
double *arrRho1 = new double[nx*ny]; 
double *arrRho2 = new double[nx*ny]; 
double *arrRho3 = new double[nx*ny]; 
double *arrRho4 = new double[nx*ny]; 
double *arrRho5 = new double[nx*ny]; 
double *arrRho6 = new double[nx*ny]; 

// Conditions for forks (looking for where these are zero)
double *amp_cond = new double[nx*ny]; 
double *phase_cond= new double[nx*ny]; 
...
double rho(double w, int index, double x, double y){
...
double rhoAB(double w, int index, double x, double y){
    double dR1x = x - R1x;
    double dR1y = y - R1y;
    double dR2x = x - R2x;
    double dR2y = y - R2y;
...
double LDOS(int ix, int iy, double x, double y, double dKx, double dKy, string sblattices){
    double dR1x = R1x-x;
    double dR1y = R1y-y;
    double dR2x = R2x-x;
    double dR2y = R2y-y;
    ...
    //gsl_complex p1 = gsl_complex_exp(gsl_complex_rect(0, dot(dKx, dKy, R1x, R1y)));
    //gsl_complex p2 = gsl_complex_exp(gsl_complex_rect(0, dot(dKx, dKy, R2x, R2y)));
    ...
    + rho(6, w, x, y)*(cos(dot(dKx, dKy, dR1x, dR1y)-thet1-thet2)+cos(dot(dKx, dKy, dR2x, dR2y)-thet1-thet2));
        ...
        gsl_complex term1 = gsl_complex_add(gsl_complex_add(gsl_complex_rect(arrRho1[iy*nx+ix] - arrRho5[iy*nx+ix], 0), \
        gsl_complex_mul_real(gsl_complex_exp(gsl_complex_rect(0, -2*thet1)),-arrRho2[iy*nx+ix])),\
        gsl_complex_mul_real(gsl_complex_exp(gsl_complex_rect(0, -thet1-thet2)),arrRho6[iy*nx+ix]));

        gsl_complex term2 = gsl_complex_add(gsl_complex_add(gsl_complex_rect(arrRho3[iy*nx+ix] - arrRho5[iy*nx+ix], 0), \
        gsl_complex_mul_real(gsl_complex_exp(gsl_complex_rect(0, -2*thet2)),-arrRho4[iy*nx+ix])),\
        gsl_complex_mul_real(gsl_complex_exp(gsl_complex_rect(0, -thet1-thet2)),arrRho6[iy*nx+ix]));
        ...
        gsl_complex_mul_real(gsl_complex_exp(gsl_complex_rect(0, -2*thet1)),-arrRho2[iy*nx+ix])),\
        gsl_complex_mul_real(gsl_complex_exp(gsl_complex_rect(0, thet2)),-2*arrRho5[iy*nx+ix])),\
        gsl_complex_mul_real(gsl_complex_exp(gsl_complex_rect(0, -thet1)),-2*arrRho6[iy*nx+ix]));

        gsl_complex term2 = gsl_complex_add(gsl_complex_add(gsl_complex_add(gsl_complex_rect(arrRho3[iy*nx+ix], 0), \
        gsl_complex_mul_real(gsl_complex_exp(gsl_complex_rect(0, 2*thet2)),-arrRho4[iy*nx+ix])),\
        gsl_complex_mul_real(gsl_complex_exp(gsl_complex_rect(0, thet2)),2*arrRho5[iy*nx+ix])),\
        gsl_complex_mul_real(gsl_complex_exp(gsl_complex_rect(0, -thet1)),2*arrRho6[iy*nx+ix]));
...
// Function which returns LDOS parts, psi1, psi2, etc.
std::array<double, 3> LDOS_Parts(int ix, int iy, double x, double y, double dKx, double dKy, string sblattices){
    double dR1x = R1x-x;
    double dR1y = R1y-y;
    double dR2x = R2x-x;
    double dR2y = R2y-y;
    ...
    + rho(6, w, x, y)*(cos(dot(dKx, dKy, dR1x, dR1y)-thet1-thet2)+cos(dot(dKx, dKy, dR2x, dR2y)-thet1-thet2));
        ...
        // Lets return the parts seperately so we can be more efficient
        // arg(whole_wave), cond1, cond2

        double cond1 = gsl_complex_abs(gsl_complex_mul(p1, term1)) - gsl_complex_abs(gsl_complex_mul(p2, term2));

        double phase_1 = gsl_complex_arg(gsl_complex_mul(p1, term1));
        double phase_2 = gsl_complex_arg(gsl_complex_mul(p2, term2));

        double cond2 = cos(phase_1 - phase_2) + 1;

        double arg_whole_wave = gsl_complex_arg(gsl_complex_add(gsl_complex_mul(C1, term1), gsl_complex_mul(C2, term2))); 

        return {arg_whole_wave, cond1, cond2};
    }
    else{
        return {0, 0, 0};
    }
...
double integrate_f_cond(double x, double y, gsl_integration_workspace * w){
    ...
    gsl_integration_qags (&F, 0.19, 0.2, 0, 1e-5, 1000,
                            w, &result, &error); 

    return result;
    }

// Integrate Rho Wrapper
// int index, double w, double x, double y
struct rho_param{
    int index;
    double x;
    double y;
...
double rho_wrapper(double w, void* params) {
    rho_param* p = static_cast<rho_param*>(params);

    return rho(w, p->index, p->x, p->y);
}

double integrate_rho(int index, double x, double y, gsl_integration_workspace * w){
    //gsl_integration_workspace * w = gsl_integration_workspace_alloc (1000);
        
    double result, error;

    //ldos_param lp = {x, y, K0, 0};
    rho_param rp = {index, x, y};

    gsl_function F;
    F.function = &rho_wrapper;
    F.params = &rp;
...
// Calculate The Rho's Over The Grid Seperately
int calculateRhos(double* d_list){
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < nx; i++){
            for (int j = 0; j < ny; j++){
                arrRho1[j*nx + i] = integrate_rho(1, cFI(i, dx, shift_x), cFI(j, dy, shift_y), w);
                arrRho2[j*nx + i] = integrate_rho(2, cFI(i, dx, shift_x), cFI(j, dy, shift_y), w);
                arrRho3[j*nx + i] = integrate_rho(3, cFI(i, dx, shift_x), cFI(j, dy, shift_y), w);
